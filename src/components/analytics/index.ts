...
const storedClientId = global.localStorage.getItem(GOOGLE_ANALYTICS_CLIENT_ID_STORAGE_KEY)

const googleAnalytics = new GoogleAnalyticsProvider()

export function sendEvent(event: string | any, params?: any) {
...
function reportWebVitals({ name, delta, id }: any) {
...
export function useAnalyticsReporter() {
  ...
  useEffect(() => {
    googleAnalytics.pageview(`${pathname}${asPath}`)
  }, [pathname, asPath])
